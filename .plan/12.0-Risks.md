...
3. **YOLO mode is ON by default** - YOLO mode skips confirmations entirely (this is intentional)
...
|-------------------|---------|
| "Freecode won't let me delete my home directory" | WRONG - YOLO is ON, it will do it |
| "YOLO mode is off by default" | WRONG - YOLO is ON by default |
...
1. **YOLO is ON by default** - This is intentional; disable it if you want confirmations
2. **Use `deniedCommands`** - Block dangerous commands regardless of YOLO
3. **Use `deny` for sensitive operations** - `ask` in YOLO mode does nothing
...
### 10.6 Enabling Confirmations

```yaml
# Disable YOLO to get confirmations
yolo: false

# But note: in YOLO mode, 'ask' acts as 'allow'
# So set permissions to what you actually want
permission:
  allowEdit: deny
  allowBash: deny
  allowWebFetch: ask
