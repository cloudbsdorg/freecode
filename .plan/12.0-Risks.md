...
3. **YOLO mode removes all guards** - YOLO mode skips confirmations entirely
...
|-------------------|---------|
| "Freecode won't let me delete my home directory" | WRONG - with `bash: allow`, it can |
| "YOLO mode just skips confirmations" | CORRECT - it removes all guards |
...
1. **Start with `ask` permissions** - Only set to `allow` when you understand the risk
2. **Review commands before allowing** - Look at what freecode wants to do
3. **Don't use YOLO mode casually** - It's for automation with trusted code
4. **Protect sensitive directories** - Use protected paths config
5. **Understand dangerous commands** - Learn what `rm -rf /*` does
6. **Use separate permissions per agent** - Not all agents need full access

### 10.6 Secure Configuration Example

```yaml
# Conservative default - asks before anything dangerous
permission:
  allowEdit: ask
  allowBash: ask
  allowWebFetch: ask
  allowTask: ask
  allowExternalDirectory: deny
  deniedCommands:
    - "rm -rf /*"
    - "> /dev/sda"
    - "mkfs"
...
# YOLO mode - only enable when you know what you're doing
yolo:
