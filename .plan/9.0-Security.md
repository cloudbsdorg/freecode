...
## 5.0 YOLO Mode (Default: Enabled)

### 5.1 YOLO Configuration

**YOLO mode is ENABLED by default.** This is intentional for automation workflows.
...
### 5.2 YOLO Default Behavior

**YOLO mode is ON by default.** This means:

```yaml
# YOLO is ON by default - no confirmations requested
yolo: true

# To disable YOLO mode (ask before actions):
yolo: false
```

### 5.3 Why YOLO is Default

1. **Automation-first:** Freecode is designed for automated workflows where confirmations would block execution
2. **Trusted environment:** Assumes user has configured permissions appropriately
3. **CI/CD friendly:** No interactive prompts in pipelines
4. **Unix philosophy:** Tools should do what you ask, not ask permission
...
```bash
# Via CLI flag
freecode run --yolo=false "fix my code"
...
**WARNING:** When YOLO is enabled (default), freecode will execute without confirmations:

```yaml
# YOLO enabled (default) - NO confirmations
yolo: true
# Will:
